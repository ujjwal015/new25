...
class Zoom_mail_sms {
    ...
    public function mailsms($send_for, $sender_details) {
        ...
        if (!empty($chk_mail_sms)) {
            ...
            if ($send_for == "online_classes") {
                ...
                $this->sendZoomOnlineClass($chk_mail_sms, $sender_details, $chk_mail_sms['template'],$chk_mail_sms['template_id']);
            } elseif ($send_for == "online_meeting") {

                $this->sendZoomMeeting($chk_mail_sms, $sender_details, $chk_mail_sms['template'],$chk_mail_sms['template_id']);
            } elseif($send_for == "zoom_online_classes_start"){


                $this->sendZoomOnlineClass($chk_mail_sms, $sender_details, $chk_mail_sms['template'],$chk_mail_sms['template_id']);
            }elseif($send_for == "zoom_online_meeting_start"){
    ...
    public function sendZoomOnlineClass($chk_mail_sms, $student_details, $template, $template_id) {
        ...
        if ($chk_mail_sms['mail'] or $chk_mail_sms['sms'] or $chk_mail_sms['notification']) {
            ...
            $student_list = $this->CI->conference_model->getStudentByClassSectionID($class_section_id);
    ...
    public function sendZoomMeeting($chk_mail_sms, $staff_details, $template,$template_id) {
