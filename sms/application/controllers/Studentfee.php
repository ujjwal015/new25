...

{
...

    {
        ...
        $data['staff_record']          = $staff_record;        
...

    {
        ...
        $this->form_validation->set_rules('collected_by', $this->lang->line('collected_by'), 'required|trim|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $staff_record = $this->staff_model->get($this->input->post('collected_by'));


            $collected_by             = $staff_record['name'] . "(" . $staff_record['employee_id'] . ")";
    ...
    public function skipstudentfee()
...
if (array_key_exists($date,$fees_data)){
 ...
 $fees_data[$date]['amt']=$fees_detail_value->amount+$fees_detail_value->amount_fine; 
  ...
  $fees_data[$date]['student_fees_deposite_ids'][]=$fee_value->student_fees_deposite_id; 
    ...
    public function feetype_report()
    {
        if (!$this->rbac->hasPrivilege('collect_fees', 'can_view')) {
            access_denied();
        }
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/finance');
        $this->session->set_userdata('subsub_menu', 'Reports/finance/feetypereport');
            $subtotal=false;
        ...
        $this->form_validation->set_rules('feetype_id', $this->lang->line('feetype_id') . " " . $this->lang->line('type'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('date_from', $this->lang->line('date_from') . " " . $this->lang->line('type'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('date_to', $this->lang->line('date_to') . " " . $this->lang->line('type'), 'trim|required|xss_clean');

        $collection          = array(); 

        if ($this->form_validation->run() == false) {
            $feetype_id = $data['results'] = array();
        } else {
            ...
            $data['results'] = $this->studentfeemaster_model->getFeeCollectionReportByFeeType($start_date, $end_date,$feetype_id, );
        ...
        $data['sch_setting'] = $this->sch_setting_detail;
        $this->load->view('layout/header', $data);
        $this->load->view('studentfee/feetype_report', $data);
        $this->load->view('layout/footer', $data);
    }
