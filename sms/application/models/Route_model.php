...
class Route_model extends MY_Model {
    ...
    public function listrouteByRouteId($id) {

        

        $query = $this->db->select("transport_route.*,vehicle_routes.vehicle_id,vehicles.vehicle_no", )
            ->join("transport_route","vehicle_routes.route_id=transport_route.id")
            ->join('vehicles',"vehicle_routes.vehicle_id=vehicles.id")
            ->where('vehicle_routes.route_id',$id)
            ->get('vehicle_routes');
    ...
    function listStudentAddress() 
    {
        
        
        $this->db->select('current_address')->from('students');

        $current_address = $this->db->group_by('current_address')->get();
        

        return $current_address->result_array();
