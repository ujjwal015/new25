...
function getStaff($id){
    $ci = &get_instance();
    $ci->load->database();
    $query = $ci->db->where(['id'=>$id])->get('staff')->row();
    if($query)
    {
...
function countPerTransLineStudent($vehicle_id){
    $ci = &get_instance();
    $ci->load->database();   

    $query = $ci->db->where(['vehroute_id'=>$vehicle_id])->get('students')->result();
...
function countStudentAreaAddress($address){
    $ci = &get_instance();
    $ci->load->database();    

    $query = $ci->db->where(['current_address'=>$address])->get('students')->result();
