...
<select class="form-control transportationarea" id="transportationarea" name="transportationarea">

<option value=""><?php echo $this->lang->line('select'); ?></option>
<?php
foreach ($transportationarea as $transportationarea_list) {
?>
<option data="<?php echo $transportationarea_list['id'] ?>" value="<?php echo $transportationarea_list['name'] ?>"><?php echo $transportationarea_list['name'] ?></option>
<?php
}
?>
</select>
...
<select class="form-control" name="transportation_line" class="transportationline" id="transportationline">
...
<input type="" name="transportation_start_date" class="form-control date">
</div>
</div>
...
<input type="" name="transportation_end_date" class="form-control date">
</div>
</div>
...
<div class="row">
    <div class="col-sm-12">
        <table class="table text-center d-none" id="vehicles-tabel">
            <tr>
                <th>Sr.No</th>
                <th>Vehicles Number</th>
                <th>Vehicles Model</th>
                <th>Vehicles Manufacture Year</th>
                <th>Select</th>
            </tr>
        </table>
    </div>
...
<td class="price" id="transportation-fee">0.00</td>
</tr>
<tr>
<td>Hostel Fee</td>
...
$(document).ready(function(){
        ...
        $.ajax({
            ...
            success:function(response){
               response=JSON.parse(response);
                     ...
                     var option= $("#transportationline option");
    ...
    //get all vechile list of selected transportation area and transportation line
    $("#transportationline").change(function(){
        var transportationarea=$(".transportationarea").val();
        var index=$(".transportationarea option:selected").index();
        var option=$(".transportationarea option");
      var transportationarea=$(option[index]).attr("data");
      
      
        var transportationline=$(this).val();
        $.ajax({
            type:"GET",
            url:"<?php echo base_url()?>Student/getAllVehicle",
            data:{
                transportationarea:transportationarea,
            ...
            success:function(response){
               ...
               response=JSON.parse(response);
               if(response.data.length>0){
                var tr=$("#vehicles-tabel tr");
                $(tr).each(function(i){
                    $(tr[i+1]).remove();
                });
                $("#vehicles-tabel").removeClass("d-none");
                $(response.data).each(function(index,data){
                     var tr=`
                 <tr>
                 ...
                 <td><input type='checkbox'  value="`+data.id+`" name="vehicles" class='select-vehicles' data="`+data.id+`"></td>
                 </tr>


                 `;
                 //restriction allow select only one vehicles
   
    //end restriction allow select only vehicles

                 $("#vehicles-tabel").append(tr);

                });


                         $(".select-vehicles").each(function(){
                       $(this).click(function(){
                        var selected=this;
                         $(".select-vehicles").each(function(){
                         ...
                         //applied transportation price 
                         var price=$("#transportationline option:selected").attr("price");
                         alert(price);
                         $("#transportation-fee").html("<i class='fa fa-inr'></i> "+Number(price).toFixed(2));
                         calculate_total_fee();
                         //end transportation price
                     });
    });
               }
               else{
                 $(tr).each(function(i){
                  ...
                  $("#vehicles-tabel").addClass("d-none");
