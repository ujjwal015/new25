...
if (!empty($online_exam_validate)) {
    ...
    ?>
            <button class="btn btn-primary print_div btn-xs" data-record-id="<?php echo $exam->id; ?>" data-loading-text="<i class='fa fa-spinner fa-spin '></i> Please Wait.."><i class="fa fa-print"></i></button>
                  <div class="aa">
    ...
    if (!$online_exam_validate->is_attempted && ($exam->is_active) && !($exam->publish_result) && $role == 'student' && strtotime(date('Y-m-d H:i:s')) >= strtotime(date($exam->exam_from)) && strtotime(date('Y-m-d H:i:s')) <= strtotime(date($exam->exam_to))) {
        ?>
                  <div class="row no-print">
                     <div class="col-xs-12">
                        <button type="button" class="btn btn-info questions" data-recordid="<?php echo $exam->id; ?>"  data-loading-text="<i class='fa fa-spinner fa-spin'></i> <?php echo $this->lang->line('please_wait'); ?>"><i class="fa fa-bullhorn"></i> <?php echo $this->lang->line('start') . " " . $this->lang->line('exam') ?></button>
                     </div>
                  </div>
                  <?php
}
    ...
    $(document).on('change','.exam_attachment',function(){
              ...
              var file = files[0];
                if (($.inArray(file.name.split('.').pop().toLowerCase(), allowed_extension) == -1) || ($.inArray(file.type,allowed_mime_type) == -1) || (allowed_upload_size <= file.size) ) {
                 errorMsg('Invalid file format or size');
